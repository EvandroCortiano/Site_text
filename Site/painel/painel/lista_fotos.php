		...
		<table>
			...
			<tr>			
				<?php 
					...
					$sql_select = "SELECT * FROM fotos ORDER BY id_foto DESC LIMIT $inicio,$registro";
					$fotos = mysql_query($sql_select);
					$total = mysql_num_rows($fotos);
					...
					while ($result = mysql_fetch_array($fotos)){ ?>
						...
						<tr class="fontdados" bgcolor="<?php echo $corlinha ?>">
							<td> <?php echo $result['link_foto']; ?></td>
							<td> <?php echo utf8_encode($result['legfoto']);?></td>
							<td align="center"><a href="delete/excluirfoto.php?id_foto=<?php echo $result['id_foto'] ?>" title="Excluir" onclick="return confirm('Tem certeza que deseja excluir esta foto?')"><img src="imagens/excluir.png" width="20px" /></a></td>
							<td align="center"><a href="upd_fotos.php?id_foto=<?php echo $result['id_foto'] ?>"><img src="imagens/atualizar.png" width="20px" /></a></td>
						</tr>
					<?php } ?>
				<tr>
